(function () {
  ...
  function drawService ($q, $filter, $timeout, $http) {
    ...
    function createRoundedRectangle (shape, shapeWidth, shapeHeight, cornerSize) {
      const roundFactor = (0.4 * cornerSize)
    ...
    function defaultBringToFront (object, scene, camera, t) {
      ...
      const timeline = new TimelineMax({
        onComplete: function () {
          const pixelAspect = window.innerHeight / camera.calculateVisibleHeight(distance - object.screen.distance)
          defer.resolve({
      ...
      const activeObjectLight = scene.activeObjectLight
      ...
      const sceneLight = scene.sceneLight
