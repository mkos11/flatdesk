(function () {
  ...
  function tableLampService (drawService) {
    ...
    function drawObject (x, y, z, colorHex) {
      const tableLampBottomGeometry = new THREE.CylinderGeometry(5, 5.5, 1, 100)
      const tableLampBottomMaterial = new THREE.MeshPhongMaterial({
        color: colorHex
      })
      const tableLampBottom = new THREE.Mesh(tableLampBottomGeometry, tableLampBottomMaterial)

      const tableLampBar1Geometry = new THREE.CylinderGeometry(0.75, 0.75, 12, 100)
      const tableLampBar1Material = new THREE.MeshPhongMaterial({
        color: 0x7f8c8d
      })
      const tableLampBar1 = new THREE.Mesh(tableLampBar1Geometry, tableLampBar1Material)
      ...
      const tableLampBar2Geometry = new THREE.CylinderGeometry(0.75, 0.75, 12, 100)
      const tableLampBar2Material = new THREE.MeshPhongMaterial({
        color: 0x7f8c8d
      })
      const tableLampBar2 = new THREE.Mesh(tableLampBar2Geometry, tableLampBar2Material)
      ...
      const tableLampBar3Geometry = new THREE.CylinderGeometry(1.25, 1.25, 3, 100)
      const tableLampBar3Material = new THREE.MeshPhongMaterial({
        color: 0x7f8c8d
      })
      const tableLampBar3 = new THREE.Mesh(tableLampBar3Geometry, tableLampBar3Material)
      ...
      const tableLampCoverGeometry = new THREE.LatheGeometry(tableLampCoverPoints, 100)
      const tableLampCoverMaterial = new THREE.MeshPhongMaterial({
        color: colorHex
      })
      const tableLampCover = new THREE.Mesh(tableLampCoverGeometry, tableLampCoverMaterial)
      tableLampCover.position.y = 20
      tableLampCover.position.z = -3
      tableLampCover.rotation.x = Math.PI - Math.PI / 4

      const tableLampBulbGeometry = new THREE.SphereGeometry(2.4, 100, 100)
      const tableLampBulbMaterial = new THREE.MeshPhongMaterial({
        color: 0xffffff,
        emissive: 0x000000,
        transparent: true,
        opacity: 0.95
      })
      const tableLampBulb = new THREE.Mesh(tableLampBulbGeometry, tableLampBulbMaterial)
      ...
      const tableLamp = new THREE.Object3D()
      ...
      const lampLight = new THREE.SpotLight(0xffffff, 0, 300)
      lampLight.angle = 1
