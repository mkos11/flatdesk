(function () {
  ...
  function postCardService (drawService, $q) {
    ...
    function drawObjectStock (x, y, z, objRotation) {
      const postCardStock = new THREE.Object3D()
      const postCardTexture = new THREE.TextureLoader().load('images/postcard.jpg')
      ...
      for (let i = 0; i <= 5; i++) {
        const value = (Math.random() / 10 + 0.9) * 0xec | 0
    ...
    function drawObject (scene, camera, x, y, z, objRotation) {
      const postCardObject = new THREE.Object3D()
      ...
      const postCard = new THREE.Mesh(new THREE.BoxBufferGeometry(16.2, 11.4, 0.2), new THREE.MeshPhongMaterial({
        color: 0xecf0f1,
        side: THREE.FrontSide,
        map: postCardTexture,
        shininess: 0,
      ...
      const hoverBox = new THREE.Mesh(new THREE.BoxBufferGeometry(16.2, 11.4, 5),
