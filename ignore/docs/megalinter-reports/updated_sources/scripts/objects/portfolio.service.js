(function () {
  ...
  function portfolioService (drawService, $q) {
    ...
    function drawObject (scene, camera, x, y, z, colorHex, angle, name, logoPath, leftPath, rightPath) {
      const portfolio = new THREE.Object3D()

      portfolio.objectWidth = 42
      portfolio.objectHeight = 29.7
      portfolio.objectThickness = 0.5

      const leftTexture = new THREE.TextureLoader().load(leftPath)
      ...
      const rightTexture = new THREE.TextureLoader().load(rightPath)
      ...
      const portfolioCover = new THREE.Object3D()

      const portfolioCoverMesh = new THREE.Mesh(
      ...
      const portfolioBackMesh = new THREE.Mesh(
      ...
      const paperGeometry = new THREE.PlaneBufferGeometry(20.25, 28.7)
      const leftPaperMaterial = new THREE.MeshPhongMaterial({
        color: 0xffffff,
        map: leftTexture
      })
      const rightPaperMaterial = new THREE.MeshPhongMaterial({
        color: 0xffffff,
        map: rightTexture
      })

      const portfolioPaper1 = new THREE.Mesh(paperGeometry, leftPaperMaterial)
      ...
      const stripeGeometry = new THREE.PlaneBufferGeometry(15, 0.5)
      ...
      const portfolioPaper2 = new THREE.Mesh(paperGeometry, rightPaperMaterial)
