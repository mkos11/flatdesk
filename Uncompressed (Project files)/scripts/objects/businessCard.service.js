(function () {
  ...
  function businessCardService (drawService, $q) {
    ...
    function drawObjectStock (x, y, z) {
      const businessCardStock = new THREE.Object3D()
      const businessCardTexture = new THREE.TextureLoader().load('images/businesscard.jpg')
      ...
      for (let i = 0; i <= 25; i++) {
        const value = (Math.random() / 4 + 0.75) * 0xec | 0
    ...
    function drawObject (scene, camera, x, y, z, objRotation) {
      ...
      const businessCard = new THREE.Mesh(new THREE.BoxBufferGeometry(9, 5, 0.1), new THREE.MeshPhongMaterial({
        color: 0xecf0f1,
        map: businessCardTexture,
        shininess: 0,
        shading: THREE.SmoothShading
      ...
      const hoverBox = new THREE.Mesh(new THREE.BoxBufferGeometry(9, 5, 5),
