(function () {
  ...
  function personalResumeService (drawService, $q) {
    ...
    function drawObject (scene, camera, x, y, z, objRotation) {
      const personalResume = new THREE.Object3D()
      personalResume.objectWidth = 21
      personalResume.objectHeight = 29.7
      personalResume.objectThickness = 0.1

      const personalResumeTextureFirst = new THREE.TextureLoader().load('images/CV.jpg')
      ...
      const personalResumeTextureSecond = new THREE.TextureLoader().load('images/CV.jpg')
      ...
      const personalResumeTextureThird = new THREE.TextureLoader().load('images/CV.jpg')
      ...
      const personalResumeFirst = new THREE.Mesh(new THREE.BoxBufferGeometry(21, 9.9, 0.1), new THREE.MeshPhongMaterial({
        color: 0xffffff,
        shininess: 0,
        map: personalResumeTextureFirst
      }))

      const personalResumeSecond = new THREE.Mesh(new THREE.BoxBufferGeometry(21, 9.9, 0.1), new THREE.MeshPhongMaterial({
        color: 0xffffff,
        shininess: 0,
        map: personalResumeTextureSecond
      }))
      const personalResumeThird = new THREE.Mesh(new THREE.BoxBufferGeometry(21, 9.9, 0.1), new THREE.MeshPhongMaterial({
        color: 0xffffff,
        shininess: 0,
        map: personalResumeTextureThird
      }))

      const hoverBox = new THREE.Mesh(new THREE.BoxBufferGeometry(21, 29.7, 5),
